...
def _incubate_novelty_candidate(state, team, spec, chk, candidate_meta, score_info, holdout_lite_err=None):
    ...
    rec = {
        "team_spec": _serialize_team_spec(
...
def _promote_from_novelty_incubator(state, cfg, gp_ctx, evaluator, island_best_fit, island_best_err, success_budget=1):
    ...
    for rank, (rec, promo_score, qual_reason, hold_reason) in enumerate(promote_candidates[:max(0, int(success_budget * 3))]):
        ...
        if motif_key and used_motifs[motif_key] >= promote_round_motif_cap:
            ...
            continue
        try:
            team = _deserialize_team_spec(rec.get("team_spec", {}), gp_ctx["pset_map"])
        except Exception:
            keep_records.append(rec)
            continue
    ...
    def validate_team_candidate(self, team, evaluator, existing_canon=None):
        ...
        if existing_canon is not None:
            if key in existing_canon or key_v2 in existing_canon:
                return {"ok": False, "reasons": ["duplicate_canonical_team"], "warnings": sorted(set(warns)), "candidate_meta": candidate_meta}
            if repair_dup_key in existing_canon:
                return {"ok": False, "reasons": ["duplicate_structural_team"], "warnings": sorted(set(warns)), "candidate_meta": candidate_meta}
