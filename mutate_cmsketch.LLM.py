...
class CMSketchEvaluator:
    ...
    def import_hard_case_state(self, state):
        if not self.hard_case_enabled:
            self.hard_case_state = {"stage_cases": [[], [], []], "absent_cases": []}
            self.hard_case_version = 0
    ...
    def _evaluate_individual_core(self, init_dex_tree, update_tree, query_tree, stage_idx: int = 2, return_case_vec: bool = False, return_hard_cases: bool = False):
        try:
            ...
            if update_pat["nonconst_path_idx"] > 0:
                penalty += update_pat["nonconst_path_idx"] * 35000

            if query_pat["nonconst_path_idx"] > 0:
                penalty += query_pat["nonconst_path_idx"] * 35000
            ...
            for s in range(stage_idx + 1):
                ...
                try:
                    if return_hard_cases:
                        ...
                        hard_case_bundle["stage_cases"][s] = [
                            {
                                "item": str(rec["item"]),
    ...
    def mine_hard_cases(self, init_dex_tree, update_tree, query_tree):
        ...
        return {"stage_cases": [[], [], []], "absent_cases": []}
...
def _inject_llm_immigrants_into_state(state, cfg, llm_team_bank):
    ...
    for rep_idx, spec in zip(repl_idx, chosen):
        ...
        try:
            fit, err, case_vec = evaluator.evaluate_individual(init_ind, update_ind, query_ind, return_case_vec=True)
        except Exception:
            fit, err = -2_000_000_000.0, 2_000_000_000.0
...
class LLMProposalEngine:
    ...
    def _build_prompt(self, phase: str, report: dict):
        return (
            f"phase={phase}; return strict json only; prefer mode=team; "
            f"output team expressions, not full python file.\ncontext={json.dumps(report, ensure_ascii=False)}"
        )
    ...
    def _call_openai_compatible(self, phase: str, report: dict):
        ...
        timeout_s = max(1.0, float(self.cfg.get("llm_timeout", 30.0)))
        if (not base_url) or (not model) or (not api_key_env):
            self.logger.warn("openai_compatible missing config, skip", base_url=bool(base_url), model=bool(model), api_key_env=api_key_env)
            return []
        api_key = os.getenv(api_key_env, "")
        if not api_key:
            self.logger.warn("openai_compatible missing api key env", api_key_env=api_key_env)
            return []
        body = {
            "model": model,
            "messages": [
                {"role": "system", "content": "Return JSON only."},
                {"role": "user", "content": self._build_prompt(phase, report)},
            ],
            "temperature": 0.2,
        }
        ...
        try:
            ...
            jt = self._extract_json(content)
            if not jt:
                self.logger.warn("openai_compatible returned non-json text")
                return []
            ...
            self.logger.warn("openai_compatible request failed", error=str(e))
            return []

    def fetch_raw_candidates(self, phase: str, report: dict):
        if self.provider == "offline_json":
            return self._load_offline()
        if self.provider == "openai_compatible":
            return self._call_openai_compatible(phase, report)
    ...
    def prepare_phase_candidates(self, phase, gp_ctx, evaluator, base_team, existing_canon, limit):
        ...
        report = self._build_report(evaluator, team=base_team, fit=fit, err=err, case_vec=vec, hard_cases=hard)
        raw = self.fetch_raw_candidates(phase, report)
        parsed = self.parse_candidate_objects(raw)
...
def replace_individual_in_state(state, idx, team, fit_err_casevec, source_meta=None):
    ...
    for k, v in list(state.items()):
        if k in {"pops", "fits", "case_vecs", "birth", "step_counter"}:
            continue
...
def _apply_llm_seed_specs_to_state(state, cfg, gp_ctx, evaluator):
    ...
    idxs = random.sample(range(pop_size), k=inject_n)
    inserted = 0
...
def _inject_llm_immigrants_with_engine(state, cfg, gp_ctx, llm_engine, candidate_specs, success_budget):
    ...
    repl_idx = sorted(range(pop_size), key=lambda j: state["birth"][j])
    inserted = 0
    ptr = 0
    for spec in candidate_specs:
        ...
        try:
            ...
            print(f"[LLM_IMMIGRANT_APPLY] idx={idx} fit={float(chk['fit']):.6f} err={float(chk['err']):.6f}", flush=True)
            inserted += 1
        except Exception:
            continue
    return state, inserted
...
def _init_island_state(cfg, island_idx: int):
    ...
    for init_t, upd_t, qry_t in triples:
        try:
            fit, err, case_vec = evaluator.evaluate_individual(init_t, upd_t, qry_t, return_case_vec=True)
        except Exception:
            fit, err = -2_000_000_000.0, 2_000_000_000.0
            case_vec = (float(evaluator.lexicase_default_bad),) * int(evaluator.lexicase_total_cases)
        fits.append((float(fit), float(err)))
    ...
    state = {
        ...
        'hard_case_state': {"version": 0, "stage_cases": [[], [], []], "absent_cases": []},
...
def _evolve_island_chunk(job):
    ...
    for _gen in range(int(local_gens)):
        ...
        for _ in range(pop_size):
            ...
            try:
                fit, err, case_vec = evaluator.evaluate_individual(
                    child['init_dex'], child['update'], child['query'], return_case_vec=True
                )
            except Exception:
                fit, err = -2_000_000_000.0, 2_000_000_000.0
                case_vec = (float(evaluator.lexicase_default_bad),) * int(evaluator.lexicase_total_cases)
    ...
    if cfg.get("hard_case_replay", False):
        try:
            ...
            state['hard_case_state'] = state.get('hard_case_state', {"version": 0, "stage_cases": [[], [], []], "absent_cases": []})
    else:
        state['hard_case_state'] = {"version": 0, "stage_cases": [[], [], []], "absent_cases": []}
...
def _migrate_island_states(island_states, mig_k: int):
    ...
    for isl in range(islands):
        ...
        src_fits = src['fits']
        top_idx = sorted(range(pop_size), key=lambda j: src_fits[j][0], reverse=True)[:elite_k]
        rest_pool = [j for j in range(pop_size) if j not in top_idx]
        rand_idx = random.sample(rest_pool, k=min(rand_k, len(rest_pool))) if rand_k > 0 and rest_pool else []
        mig_idx = top_idx + rand_idx

        migrants = []
        for j in mig_idx:
            migrants.append((
                copy.deepcopy(src['pops']['init_dex'][j]),
                copy.deepcopy(src['pops']['update'][j]),
                copy.deepcopy(src['pops']['query'][j]),
                src['fits'][j],
            ))

        dst_fits = dst['fits']
        dst_birth = dst['birth']
        repl_idx = sorted(range(pop_size), key=lambda j: dst_birth[j])[:len(migrants)]

        for rep_idx, mig in zip(repl_idx, migrants):
            dst['pops']['init_dex'][rep_idx] = mig[0]
            dst['pops']['update'][rep_idx] = mig[1]
            dst['pops']['query'][rep_idx] = mig[2]
            dst['fits'][rep_idx] = mig[3]
            dst['birth'][rep_idx] = int(dst['step_counter'])
            dst['step_counter'] = int(dst['step_counter']) + 1
...
def evolve_cmsketch(
    ...
    try:
        ...
        print(f"初始最佳适应度(-AAE): {best_fitness:.6f} (AAE={best_error:.6f})")
        ...
        while gens_done < int(generations):
            ...
            print(
                f"平均适应度(-AAE): {avg_fit:.6f} (平均AAE={avg_err:.6f})，最佳: {best_fitness:.6f} (AAE={best_error:.6f})",
                flush=True,
            )
            if improved:
                no_improve_chunks = 0
            ...
            if (
                ...
                try:
                    ...
                    if candidate_specs and budget > 0:
                        ...
                        remain = budget
                        for st in island_states:
                            st, inserted = _inject_llm_immigrants_with_engine(
                                st,
                                cfg,
                                gp_ctx_main,
                                llm_engine,
                                candidate_specs,
                                success_budget=remain,
                            )
                            total_injected += int(inserted)
                            remain = max(0, remain - int(inserted))
                            new_states.append(st)
                            if remain <= 0:
                                new_states.extend(island_states[len(new_states):])
                                break
