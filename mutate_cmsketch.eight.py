...
def _pick_spec_family_injection_sort_key(spec, profile, family_hist=None, pset_map=None, target_parts=None):
    ...
    return (-target_hits, -allowed_hits, exact_count, comp_count, str(_spec_family_tag(spec, pset_map=pset_map)))
...
def _novelty_family_metrics(team_or_spec, family_hist, target_parts, pset_map=None):
...
def _novelty_family_gate(team_or_spec, family_hist, target_parts, pset_map=None, min_match=2, min_distance=1):
    metrics = _novelty_family_metrics(team_or_spec, family_hist, target_parts, pset_map=pset_map)
    reasons = []
    if int(metrics["target_match"]) < int(min_match):
        reasons.append(f"novelty_family_match_too_low:{metrics['target_match']}<{int(min_match)}")
    if int(metrics["mainstream_distance"]) < int(min_distance):
        reasons.append(f"novelty_family_distance_too_low:{metrics['mainstream_distance']}<{int(min_distance)}")
...
def _compute_novelty_score(chk_fit, chk_err, island_best_fit, island_best_err, family_metrics, saturation_metrics, failure_summary, holdout_err=None):
    ...
    score = 18.0 * float(target_match) + 10.0 * float(mainstream_distance)
    score -= 14.0 * float(exact_count)
    score -= min(18.0, 0.75 * float(component_count))
    score -= 7.0 * float(len(failure_buckets))
    ...
    profiles = [
        ...
        },
        {
            ...
            "allowed_family_labels": {
                "init_dex": {"symmetric-init", "asymmetric-init", "slice-hash-init", "hybrid-slice-init", "layered-init"},
                "update": {"triple-write", "read-before-write", "conditional-write", "stateful-write", "rescue-write"},
                "query": {"median-query", "avg-query", "state-gated-query", "free-query", "mixed-aggregator-query", "fallback-query"},
            },
            "innovation_family_labels": {
                "init_dex": {"hybrid-slice-init", "layered-init"},
                "update": {"conditional-write", "stateful-write", "rescue-write"},
...
class LLMProposalEngine:
    ...
    def _build_prompt(self, phase: str, report: dict, repair_feedback: str = ""):
        ...
        if use_single_tree:
            ...
            if candidate_channel == "repair":
                lines.append("Repair channel: prioritize fixing current failure buckets and keep the overall sketch family stable.")
        else:
            lines.extend([
            ...
            if candidate_channel == "novelty":
                lines.append("Novelty channel: prioritize matching the target family and being structurally different from saturated current families over tiny short-term score gains.")
            if candidate_channel == "irregular_novelty":
                lines.append("Irregular novelty channel: propose a genuinely innovative sketch family, not a tiny local variation.")
                lines.append("Irregular novelty requirements: team-level proposal only; do NOT use a single-tree style local patch mindset.")
                lines.append("Prefer asymmetric multi-path roles, layered or hybrid init structure, stateful/overflow-like update logic, and fallback or mixed-aggregator query structure.")
                lines.append("Avoid collapsing back to plain symmetric-init/triple-write/min-query unless the structure is still clearly more innovative and cross-family.")
    ...
    def prepare_phase_candidates(self, phase, gp_ctx, evaluator, base_team, existing_canon, limit, extra_prompt_hints=None, family_guidance=None, force_single_tree=None, force_single_tree_target=None, target_funcs_override=None, candidate_channel="", adaptive_reason=""):
        ...
        def _consume_raw(raw_list):
            nonlocal n_materialized, n_validated, n_evaluated, total_raw, total_parsed, total_team_parsed, total_single_parsed, out, seen, failed_records
            ...
            for obj in parsed_single:
                ...
                if perr:
                    failed_records.append({"stage": "materialize_single_tree", "reasons": list(perr)})
                    continue
                n_materialized += 1
                chk = self.validate_team_candidate(team, evaluator, existing_canon=seen)
                if not chk.get("ok", False):
                    failed_records.append({"stage": "validate_single_tree", "reasons": list(chk.get("reasons", []))})
                    continue
                n_validated += 1
                n_evaluated += 1
                if chk.get("warnings"):
            ...
            if channel == "novelty":
                ...
                if not fam_ok:
                    ...
                    llm_engine.logger.info(
                        ...
                        family_distance=int(fam_metrics.get("mainstream_distance", 0)),
                    )
                    continue
                ...
                if novelty_subtype == "irregular":
                    ...
                    score_info["innovation_bonus"] = float(irregular_bonus_info.get("bonus", 0.0))
                    score_info["innovation_reasons"] = list(irregular_bonus_info.get("reasons", []))
                min_score = float(cfg.get("llm_irregular_min_score", 16.0) if novelty_subtype == "irregular" else cfg.get("llm_novelty_min_score", 20.0))
