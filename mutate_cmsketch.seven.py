...
class CMSketchEvaluator:
    def __init__(self,
        ...
        self.debug_stats = {
            ...
            "dyn_path_state_only_total": 0,
            "dyn_path_count_only_total": 0,
            "dyn_path_mixed_total": 0,
            "dyn_path_nested_total": 0,
            "dyn_path_depth_total": 0,
            "dyn_path_max_depth": 0,
    ...
    def _ast_pattern_summary(self, which: str, root):
        ...
        out = {
            ...
            "nonconst_path_idx": 0,
            "bad_write_context": 0,
            "dynamic_path_state_only": 0,
            "dynamic_path_count_only": 0,
            "dynamic_path_mixed": 0,
            "dynamic_path_nested": 0,
            "dynamic_path_depth_sum": 0,
            "dynamic_path_max_depth": 0,
        }

        read_like = {"query_count", "query_state", "query_date", "cnt_rdstate"}
        state_like = {"query_state", "cnt_rdstate"}
        count_like = {"query_count"}
        query_like = {"query_date"}
        ...
        def _node_depth(nd):
            ...
            mx = 0
            for ch in nd["children"]:
                mx = max(mx, _node_depth(ch))
            return 1 + mx

        def _collect_path_features(nd):
            feats = {
                "uses_state": False,
                "uses_count": False,
                "uses_query_date": False,
                "nested_read": 0,
            }

            def walk(cur, parent_read=False):
                if cur["kind"] == "term":
                    return
                name = cur["name"]
                is_read = name in read_like
                if name in state_like:
                    feats["uses_state"] = True
                if name in count_like:
                    feats["uses_count"] = True
                if name in query_like:
                    feats["uses_query_date"] = True
                if is_read and parent_read:
                    feats["nested_read"] += 1
        ...
        def _path_bucket(feats):
            if feats["uses_state"] and (not feats["uses_count"]) and (not feats["uses_query_date"]):
                return "state_only"
            if feats["uses_count"] and (not feats["uses_state"]) and (not feats["uses_query_date"]):
                return "count_only"
            return "mixed"
        ...
        def walk(nd, parent_name=None, child_pos=None):
            ...
            if which in {"update", "query"} and name in idx_pos_map:
                ...
                if cv is None:
                    ...
                    feats = _collect_path_features(idx_node)
                    depth = int(_node_depth(idx_node))
                    bucket = _path_bucket(feats)
                    if bucket == "state_only":
                        out["dynamic_path_state_only"] += 1
                    elif bucket == "count_only":
                        out["dynamic_path_count_only"] += 1
                    else:
                        out["dynamic_path_mixed"] += 1
                    if feats["nested_read"] > 0:
                        out["dynamic_path_nested"] += 1
                    out["dynamic_path_depth_sum"] += depth
                    out["dynamic_path_max_depth"] = max(out["dynamic_path_max_depth"], depth)
                else:
                    try:
    ...
    def _evaluate_individual_core(self, init_dex_tree, update_tree, query_tree, stage_idx: int = 2, return_case_vec: bool = False, return_hard_cases: bool = False):
        try:
            ...
            self.debug_stats["dyn_path_state_only_total"] += int(update_pat.get("dynamic_path_state_only", 0)) + int(query_pat.get("dynamic_path_state_only", 0))
            self.debug_stats["dyn_path_count_only_total"] += int(update_pat.get("dynamic_path_count_only", 0)) + int(query_pat.get("dynamic_path_count_only", 0))
            self.debug_stats["dyn_path_mixed_total"] += int(update_pat.get("dynamic_path_mixed", 0)) + int(query_pat.get("dynamic_path_mixed", 0))
            self.debug_stats["dyn_path_nested_total"] += int(update_pat.get("dynamic_path_nested", 0)) + int(query_pat.get("dynamic_path_nested", 0))
            self.debug_stats["dyn_path_depth_total"] += int(update_pat.get("dynamic_path_depth_sum", 0)) + int(query_pat.get("dynamic_path_depth_sum", 0))
            self.debug_stats["dyn_path_max_depth"] = max(
                int(self.debug_stats.get("dyn_path_max_depth", 0)),
                int(update_pat.get("dynamic_path_max_depth", 0)),
                int(query_pat.get("dynamic_path_max_depth", 0)),
            )
            ...
            if update_pat["nonconst_path_idx"] > 0:
                penalty += update_pat["nonconst_path_idx"] * 4000
                penalty += int(update_pat.get("dynamic_path_state_only", 0)) * 4000
                penalty += int(update_pat.get("dynamic_path_count_only", 0)) * 9000
                penalty += int(update_pat.get("dynamic_path_mixed", 0)) * 14000
                penalty += int(update_pat.get("dynamic_path_nested", 0)) * 35000
                if int(update_pat.get("dynamic_path_max_depth", 0)) > 1:
                    penalty += (int(update_pat.get("dynamic_path_max_depth", 0)) - 1) * 12000

            if query_pat["nonconst_path_idx"] > 0:
                penalty += query_pat["nonconst_path_idx"] * 3000
                penalty += int(query_pat.get("dynamic_path_state_only", 0)) * 2000
                penalty += int(query_pat.get("dynamic_path_count_only", 0)) * 6000
                penalty += int(query_pat.get("dynamic_path_mixed", 0)) * 10000
                penalty += int(query_pat.get("dynamic_path_nested", 0)) * 20000
                if int(query_pat.get("dynamic_path_max_depth", 0)) > 1:
                    penalty += (int(query_pat.get("dynamic_path_max_depth", 0)) - 1) * 8000
...
def _classify_init_family(tree):
    ...
    hash_kind_n = int("select_hash(" in txt) + int("hash_salt(" in txt) + int("hash_on_slice(" in txt)
    if has_slice:
...
def _classify_update_family(tree):
    ...
    has_read_before = ("query_count(" in txt) and (("write_count(" in txt) or ("writecount_if(" in txt))
    if has_read_before:
...
def _get_island_profile(cfg, island_idx: int):
    ...
    profiles = [
        ...
        },
        {
            ...
            "allowed_family_labels": {
                "init_dex": {"asymmetric-init", "slice-hash-init"},
                "update": {"triple-write", "read-before-write"},
                "query": {"min-query", "median-query", "avg-query"},
            },
        ...
        },
        {
            ...
            "init_p_llm_seed": 0.15 if llm_on else 0.0,
            "allowed_family_labels": {
                "init_dex": {"symmetric-init", "asymmetric-init", "slice-hash-init"},
                "update": {"triple-write", "read-before-write"},
                "query": {"median-query", "avg-query", "state-gated-query", "free-query"},
            },
            "mutation_weights": {"init_dex": 0.15, "update": 0.20, "query": 0.65},
            "reset_weights": {"init_dex": 0.15, "update": 0.20, "query": 0.65},
            "family_jump_prob": 0.18,
            "family_jump_weights": {"init_dex": 0.10, "update": 0.10, "query": 0.80},
        },
        {
            "name": "update_explore",
            "init_p_skeleton": 0.58,
            ...
            "allowed_family_labels": {
                "init_dex": {"symmetric-init", "asymmetric-init"},
                "update": {"read-before-write", "double-write", "mixed-write", "triple-write"},
                "query": {"min-query", "median-query", "avg-query"},
            },
...
def _apply_llm_seed_specs_to_state(state, cfg, gp_ctx, evaluator):
    ...
    if len(specs) > 1:
        specs = sorted(
            specs,
            key=lambda sp: (-_pick_spec_family_match_score(sp, island_profile, gp_ctx["pset_map"]), str(sp.get("family_tag", "")), str(sp.get("source", "")))
        )
    ...
    for rank, idx in enumerate(idxs):
        ...
        try:
            ...
            replace_individual_in_state(
                ...
                team,
                (fit, err, case_vec),
                source_meta={"phase": "seed", "source": spec.get("source", "offline_json"), "rationale": spec.get("rationale", ""), "family_tag": spec.get("family_tag", "")},
                fec_key=fec_key,
            )
    ...
    for spec in candidate_specs:
        ...
        try:
            ...
            replace_individual_in_state(
                ...
                source_meta={"phase": "stagnation", "source": spec.get("source", "offline_json"), "rationale": spec.get("rationale", ""), "family_tag": spec.get("family_tag", "")},
                fec_key=chk["key"],
            )
...
def _append_historical_best_to_holdout_candidates(candidates, best_team, best_fitness, best_error):
    ...
    for cand in candidates:
        try:
            seen.add(_team_expr_key_for_holdout(cand["team"]))
        except Exception:
            continue
...
def _rerank_candidates_with_real_holdout(args_dict, candidates, log_prefix="[STAGE2_HOLDOUT]"):
    ...
    for rank, cand in enumerate(candidates, start=1):
        ...
        scored.append(rec)
        print(
            f"{log_prefix} rank={rank} stage1_fit={rec['stage1_fit']:.6f} stage1_err={rec['stage1_err']:.2f} holdout_err={rec['holdout_err']:.2f}",
            flush=True,
        )
    ...
    if chosen is not None:
        print(
            f"{log_prefix} chosen_holdout_err={chosen['holdout_err']:.2f} chosen_stage1_fit={chosen['stage1_fit']:.6f} chosen_stage1_err={chosen['stage1_err']:.2f}",
            flush=True,
        )
...
def _init_island_state(cfg, island_idx: int):
    ...
    print(
        ...
        f"dyn_state={dbg.get('dyn_path_state_only_total',0)} dyn_count={dbg.get('dyn_path_count_only_total',0)} dyn_mixed={dbg.get('dyn_path_mixed_total',0)} dyn_nested={dbg.get('dyn_path_nested_total',0)} dyn_max_depth={dbg.get('dyn_path_max_depth',0)} "
...
def _evolve_island_chunk(job):
    ...
    print(
        ...
        f"hard_illegal={dbg['hard_illegal']} real_write_zero={dbg['real_write_zero']} "
        f"query_date_zero={dbg['query_date_zero']} penalty_dom={dbg['penalty_dominates']} "
        f"early_cut={dbg['early_return_cut']} dyn_state={dbg.get('dyn_path_state_only_total',0)} dyn_count={dbg.get('dyn_path_count_only_total',0)} dyn_mixed={dbg.get('dyn_path_mixed_total',0)} dyn_nested={dbg.get('dyn_path_nested_total',0)} dyn_max_depth={dbg.get('dyn_path_max_depth',0)} "
    ...
    try:
        with open(log_path, "w", encoding="utf-8") as lf, \
            ...
            if bool(args_dict.get("stage2_holdout_enable", False)) and top_candidates:
                holdout_pool = _append_historical_best_to_holdout_candidates(
