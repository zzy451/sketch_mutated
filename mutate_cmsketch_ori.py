...
class CMSketchEvaluator:
    def __init__(self,
                 ...
                 penalty_relax_until: float = 0.35,
                 penalty_tighten_from: float = 0.75,
                 absent_error_weight: float = 0.35):
        ...
        self.evolution_progress = min(1.0, max(0.0, float(evolution_progress)))
        self.penalty_relax_until = min(1.0, max(0.0, float(penalty_relax_until)))
        self.penalty_tighten_from = min(1.0, max(self.penalty_relax_until + 1e-6, float(penalty_tighten_from)))
        self.absent_error_weight = min(1.0, max(0.0, float(absent_error_weight)))
        ...
        self.debug_stats = {
            ...
            "bad_write_value_total": 0,
            "query_path_unstable_total": 0.0,
            "query_update_path_mismatch_total": 0.0,
    ...
    def _ast_pattern_summary(self, which: str, root):
        ...
        out = {
            ...
            "nonconst_path_idx": 0,
            "bad_write_context": 0,
            "bad_write_value_shape": 0,
            "query_value_missing_reads": 0,
            "query_constant_branch": 0,
        }

        def walk(nd, parent_name=None, child_pos=None, under_if=False):
            if nd["kind"] == "term":
                return

            name = nd["name"]
            cur_under_if = bool(under_if) or (name in {"if_then_else", "if_then_else_int"})
            ...
            # update: 写副作用不应该藏在比较/算术/切片里；
            # 另外，只要写操作出现在 if_then_else / if_then_else_int 的任意子树中，
            # 都视为 bad_write_context（因为 Python/GP 求值时参数会先求值）。
            ...
            # update: write_count / update_count 的写入值不应该是布尔/比较型/状态位读
            if which == "update" and name in {"write_count", "update_count"} and len(nd["children"]) == 3:
                val_node = nd["children"][2]
                val_cv = self._const_of(val_node)
                val_info = self._ast_collect_names(val_node)

                bad_value_shape = False

                if isinstance(val_cv, bool):
                    bad_value_shape = True

                if (
                    int(val_info["names"]["lt"]) > 0
                    or int(val_info["names"]["gt"]) > 0
                    or int(val_info["names"]["eq"]) > 0
                    or int(val_info["names"]["query_state"]) > 0
                    or int(val_info["names"]["cnt_rdstate"]) > 0
                ):
                    bad_value_shape = True

                if bad_value_shape:
                    out["bad_write_value_shape"] += 1

            # query: base_sel 的三个值分支里，至少两路应该是真实 query_date 读取
            if which == "query" and name == "base_sel" and len(nd["children"]) == 4:
                real_read_branches = 0
                for ch in nd["children"][1:4]:
                    info = self._ast_collect_names(ch)
                    if int(info["names"]["query_date"]) > 0:
                        real_read_branches += 1
                    if self._const_of(ch) is not None:
                        out["query_constant_branch"] += 1

                if real_read_branches < 2:
                    out["query_value_missing_reads"] += (2 - real_read_branches)

            for i, ch in enumerate(nd["children"]):
                walk(ch, name, i, cur_under_if)
    ...
    def _penalty_progress_alpha(self) -> float:
        p = min(1.0, max(0.0, float(getattr(self, "evolution_progress", 0.0))))
        lo = min(1.0, max(0.0, float(getattr(self, "penalty_relax_until", 0.35))))
        hi = min(1.0, max(lo + 1e-9, float(getattr(self, "penalty_tighten_from", 0.75))))
        if p <= lo:
            return 0.0
        if p >= hi:
            return 1.0
        return (p - lo) / max(1e-9, hi - lo)

    def _blend_penalty(self, early_value: float, late_value: float) -> float:
        alpha = self._penalty_progress_alpha()
        return float(early_value) + (float(late_value) - float(early_value)) * float(alpha)
    ...
    def _runtime_query_semantic_penalty(self, runtime_trace, query_eff):
        out = {
            "path_unstable": 0.0,
            "update_mismatch": 0.0,
            "penalty": 0.0,
        ...
        recs = list(runtime_trace.get("query_call_records", []))
        if not recs:
            return out

        sigs = [tuple(rec.get("paths", ())) for rec in recs if rec.get("paths")]
        if sigs:
            cnt = Counter(sigs)
            dominant = max(cnt.values())
            total = len(sigs)
            dominant_ratio = float(dominant) / max(1, total)
            uniq_n = len(cnt)

            unstable_score = 0.0
            if uniq_n > 1:
                unstable_score += float(uniq_n - 1)
            if dominant_ratio < 0.85:
                unstable_score += max(0.0, 0.85 - dominant_ratio) * 6.0

            try:
                qread_calls = int(query_eff.get("query_date_calls", 0))
            except Exception:
                qread_calls = 0
            if qread_calls >= 2:
                avg_card = sum(len(s) for s in sigs) / max(1, len(sigs))
                if avg_card < 2.0:
                    unstable_score += float(2.0 - avg_card)

            if unstable_score > 0.0:
                out["path_unstable"] = float(unstable_score)
                out["penalty"] += float(unstable_score) * self._blend_penalty(700.0, 5500.0)

        update_item_paths = runtime_trace.get("update_item_paths", {})
        mismatch_score = 0.0
        compared = 0
        for rec in recs:
            try:
                expected = int(rec.get("expected", 0))
            except Exception:
                expected = 0
            if expected <= 0:
                continue
            qsig = set(rec.get("paths", ()))
            if not qsig:
                continue
            item = str(rec.get("item", ""))
            upsig = update_item_paths.get(item)
            if not upsig:
                continue
            upsig = {int(x) % 3 for x in upsig}
            compared += 1
            inter = qsig & upsig
            overlap = float(len(inter)) / max(1, len(qsig))
            if overlap <= 0.0:
                mismatch_score += 1.0
            elif overlap < 0.5:
                mismatch_score += (0.5 - overlap) * 1.5

        if compared > 0 and mismatch_score > 0.0:
            norm = float(mismatch_score) / max(1, compared)
            out["update_mismatch"] = norm
            out["penalty"] += norm * self._blend_penalty(3500.0, 26000.0)
    ...
    def _infer_failure_buckets(self, init_pat, update_pat, query_pat, update_eff, query_eff):
        ...
        if int(update_pat.get("bad_write_value_shape", 0)) > 0:
            buckets.append("bad_write_value")

        if (
            int(query_pat.get("query_value_missing_reads", 0)) > 0
            or int(query_pat.get("query_constant_branch", 0)) > 0
        ):
            buckets.append("query_structure")
    ...
    def _query_error_on_items(self, query_func, expected_freq, items=None, max_items=None, return_records: bool = False, runtime_trace=None, trace_label: str = ""):
        if items is None:
            items = list(expected_freq.items())
            items.sort(key=lambda kv: kv[1], reverse=True)
        ...
        for item, expected in norm_items:
            ...
            if isinstance(runtime_trace, dict):
                runtime_trace["current_query_item"] = item_key
                runtime_trace["current_query_paths"] = []
            estimated = query_func(item)
            if isinstance(runtime_trace, dict):
                sig = self._path_signature(runtime_trace.get("current_query_paths"))
                runtime_trace.setdefault("query_call_records", []).append({
                    "item": item_key,
                    "expected": int(expected),
                    "paths": sig,
                    "label": str(trace_label or ""),
                })
                runtime_trace["current_query_item"] = None
                runtime_trace["current_query_paths"] = None
    ...
    def _evaluate_individual_core(self, init_dex_tree, update_tree, query_tree, stage_idx: int = 2, return_case_vec: bool = False, return_hard_cases: bool = False):
        ...
        try:
            ...
            penalty = 0.0
            stage_query_total = 0.0
            absent_query_total = 0.0
            total_query_items = 0
            ...
            struct_scale = self._blend_penalty(0.35, 1.00)
            light_scale = self._blend_penalty(0.20, 1.00)
            runtime_scale = self._blend_penalty(0.75, 1.00)

            hash_calls = int(init_info.get("hash_calls", 0))

            # ---- 轻量结构引导：前期宽松，后期收紧 ----
            if not init_info["root_ok"]:
                penalty += self._blend_penalty(20000.0, 65000.0)
            if hash_calls == 0:
                penalty += self._blend_penalty(12000.0, 35000.0)
            elif hash_calls < 2:
                penalty += (2 - hash_calls) * self._blend_penalty(3000.0, 9000.0)

            if not update_info["root_ok"]:
                penalty += self._blend_penalty(15000.0, 50000.0)
            if not query_info["root_ok"]:
                penalty += self._blend_penalty(15000.0, 50000.0)

            # ---- 真正硬非法：保留 immediate cut ----
            if init_ast_chk["hard_illegal"] or update_ast_chk["hard_illegal"] or query_ast_chk["hard_illegal"]:
                ...
                hard_penalty = float(penalty + self._blend_penalty(260000.0, 420000.0))
                self.debug_stats["early_return_cut"] += 1
                self.debug_stats["penalty_sum"] += hard_penalty
                self.debug_stats["total_error_sum"] += hard_penalty
                return _ret(hard_penalty)

            # ---- 退化检测：保留代表性信号，不重复重罚 ----
            if not update_eff["depends_on_e"]:
                penalty += self._blend_penalty(6000.0, 22000.0)
            ...
            if update_eff["real_write_calls"] == 0:
                ...
                if update_eff["conditional_write_calls"] == 0:
                    penalty += self._blend_penalty(35000.0, 110000.0)
                else:
                    penalty += self._blend_penalty(18000.0, 55000.0)

            if query_eff["query_date_calls"] == 0:
                self.debug_stats["query_date_zero"] += 1
                penalty += self._blend_penalty(40000.0, 120000.0)
            ...
            self.debug_stats["bad_write_value_total"] += int(update_pat.get("bad_write_value_shape", 0))

            if init_pat["nonconst_hash_idx"] > 0:
                penalty += int(init_pat["nonconst_hash_idx"]) * self._blend_penalty(1200.0, 7000.0)

            if update_pat["nonconst_path_idx"] > 0:
                penalty += int(update_pat["nonconst_path_idx"]) * self._blend_penalty(1800.0, 11000.0)

            if query_pat["nonconst_path_idx"] > 0:
                penalty += int(query_pat["nonconst_path_idx"]) * self._blend_penalty(1800.0, 18000.0)

            if update_pat["bad_write_context"] > 0:
                penalty += int(update_pat["bad_write_context"]) * self._blend_penalty(7000.0, 36000.0)

            if int(update_pat.get("bad_write_value_shape", 0)) > 0:
                penalty += int(update_pat["bad_write_value_shape"]) * self._blend_penalty(2000.0, 12000.0)

            if int(query_pat.get("query_value_missing_reads", 0)) > 0:
                penalty += int(query_pat["query_value_missing_reads"]) * self._blend_penalty(1200.0, 7500.0)

            if int(query_pat.get("query_constant_branch", 0)) > 0:
                penalty += int(query_pat["query_constant_branch"]) * self._blend_penalty(150.0, 1200.0)
            ...
            if avg_unique_locs < 2.0:
                penalty += float((2.0 - avg_unique_locs) * self._blend_penalty(4000.0, 18000.0))
            ...
            runtime_trace = {
                ...
                "current_update_paths": None,
                "current_query_item": None,
                "current_query_paths": None,
                "item_cap": 512,
            }
            ...
            def create_pset_update(count_matrices, overflow_matrices, exec_stats):
                ...
                def write_count(e: str, i: int, delta: int) -> int:
                    ...
                    if runtime_trace.get("current_update_paths") is not None:
                        runtime_trace["current_update_paths"].append(z)
            ...
            def create_pset_query(count_matrices, overflow_matrices, exec_stats):
                ...
                def cnt_rdstate(e: str, i: int) -> bool:
                    ...
                    y = vars[1] % 102
                    z = i % 3
                    if runtime_trace.get("current_query_paths") is not None:
                        runtime_trace["current_query_paths"].append(z)
            ...
            for s in range(stage_idx + 1):
                ...
                try:
                    for item in cur_stream:
                        item_key = str(item)
                        runtime_trace["current_update_item"] = item_key
                        runtime_trace["current_update_paths"] = []
                        update_func(item)
                        sig = self._path_signature(runtime_trace.get("current_update_paths"))
                        if sig:
                            upmap = runtime_trace.setdefault("update_item_paths", {})
                            if item_key in upmap or len(upmap) < int(runtime_trace.get("item_cap", 512)):
                                upmap.setdefault(item_key, set()).update(sig)
                        runtime_trace["current_update_item"] = None
                        runtime_trace["current_update_paths"] = None
                except Exception:
                ...
                base_min_ops = float(self.kpart_upd_min[s])
                eff_min_ops = self._blend_penalty(max(0.75, base_min_ops * 0.65), base_min_ops)
                if avg_upd_ops < eff_min_ops:
                    penalty += float((eff_min_ops - avg_upd_ops) * self._blend_penalty(14000.0, float(self.kpart_upd_penalty_scale[s])))

                stage_items = self._get_stage_eval_items(s, stage_expected[s])

                try:
                    if return_hard_cases:
                        stage_err, stage_avg_err, stage_qn, hard_records = self._query_error_on_items(
                            query_func,
                            stage_expected[s],
                            items=stage_items,
                            return_records=True,
                            runtime_trace=runtime_trace,
                            trace_label=f"stage{s}",
                        )
                        top_stage_records = [
                            {
                                "item": str(rec["item"]),
                        ...
                        stage_err, stage_avg_err, stage_qn = self._query_error_on_items(
                            query_func,
                            stage_expected[s],
                            items=stage_items,
                ...
                stage_query_total += float(stage_err)
                total_query_items += int(stage_qn)

                err_thr = self.kpart_avg_err_thresh[s]
                if err_thr is not None:
                    eff_err_thr = self._blend_penalty(float(err_thr) * 1.35, float(err_thr))
                    if stage_avg_err > eff_err_thr:
                        penalty += float(stage_avg_err - eff_err_thr) * self._blend_penalty(0.0, 8.0)
            ...
            try:
                if return_hard_cases:
                    absent_err, absent_avg_err, absent_qn, absent_records = self._query_error_on_items(
                        query_func,
                    ...
                    absent_err, absent_avg_err, absent_qn = self._query_error_on_items(
                        query_func,
                        stage_expected[min(stage_idx, len(stage_expected) - 1)],
                        items=absent_eval_items,
                        return_records=False,
                ...
                absent_query_total = float(absent_err)
                total_query_items += int(absent_qn)
            except Exception:
                absent_query_total = 0.0
                absent_avg_err = 0.0
                absent_qn = 0
            ...
            query_runtime_sem = self._runtime_query_semantic_penalty(runtime_trace, query_eff)
            if float(query_runtime_sem.get("path_unstable", 0.0)) > 0.0:
                self.debug_stats["query_path_unstable_total"] += float(query_runtime_sem["path_unstable"])
            if float(query_runtime_sem.get("update_mismatch", 0.0)) > 0.0:
                self.debug_stats["query_update_path_mismatch_total"] += float(query_runtime_sem["update_mismatch"])
            penalty += float(query_runtime_sem.get("penalty", 0.0))

            avg_qry_reads = exec_stats["qry_reads"] / max(1, total_query_items)
            eff_min_qreads = self._blend_penalty(0.85, 2.0)
            if avg_qry_reads < eff_min_qreads:
                penalty += float((eff_min_qreads - avg_qry_reads) * self._blend_penalty(12000.0, 32000.0))

            total_query_error = float(stage_query_total + float(self.absent_error_weight) * absent_query_total)
            total_error = float(total_query_error + penalty)
            self.debug_stats["penalty_sum"] += float(penalty)
            self.debug_stats["query_error_sum"] += float(total_query_error)
            self.debug_stats["total_error_sum"] += float(total_error)
            if float(penalty) > float(total_query_error):
...
class Stage1MultiProxyEvaluator:
    ...
    def __init__(self, cfg):
        ...
        train_modes = list(self.cfg.get("stage1_proxy_modes", []))
        if not train_modes:
            train_modes = [str(self.cfg.get("proxy_mode", "proxy_balanced"))]
        self.stage1_proxy_modes = [str(x) for x in train_modes]
    ...
    def _build_eval_specs(self):
        ...
        specs = []
        weights = {m: 1.0 / max(1, len(self.stage1_proxy_modes)) for m in self.stage1_proxy_modes}
        for idx, mode in enumerate(self.stage1_proxy_modes):
            ...
            specs.append({
                "name": f"train:{mode}",
                "mode": str(mode),
                "weight": float(weights.get(mode, 0.0)),
                "evaluator": ev,
            })
...
def _collect_recent_failure_hints_from_states(island_states):
    ...
    for st in island_states or []:
        ...
        if isinstance(recent_hist, list):
            for dbg in recent_hist:
                ...
                metric_counts["bad_write_value"] += int(dbg.get("bad_write_value_total", 0))
    ...
    for key in ("query_date_zero", "bad_write_ctx", "bad_write_value", "nonconst_path", "nonconst_hash", "real_write_zero"):
    ...
    text_map = {
        ...
        "bad_write_value": (
            "Avoid boolean/comparison/state-read expressions as direct write values.",
            "Make write values look like stable count-like integers instead of boolean/state outputs.",
        ),
    }
...
def _make_evaluator_from_cfg(cfg):
    ...
    if use_stage1_multi_proxy:
        ...
        try:
            ev.primary.evolution_progress = min(1.0, max(0.0, float(cfg.get("evolution_progress", 0.0))))
            ev.primary.penalty_relax_until = min(1.0, max(0.0, float(cfg.get("penalty_relax_until", 0.35))))
            ev.primary.penalty_tighten_from = min(1.0, max(ev.primary.penalty_relax_until + 1e-6, float(cfg.get("penalty_tighten_from", 0.75))))
            ev.primary.absent_error_weight = min(1.0, max(0.0, float(cfg.get("absent_error_weight", 0.35))))
        except Exception:
            pass
    else:
        ev = CMSketchEvaluator(
            ...
            evolution_progress=float(cfg.get("evolution_progress", 0.0)),
            penalty_relax_until=float(cfg.get("penalty_relax_until", 0.35)),
            penalty_tighten_from=float(cfg.get("penalty_tighten_from", 0.75)),
            absent_error_weight=float(cfg.get("absent_error_weight", 0.35)),
        )
...
def _evolve_island_chunk(job):
    ...
    print(
        ...
        f"bad_write_val_total={dbg.get('bad_write_value_total', 0)} "
        f"penalty_avg={dbg['penalty_sum'] / eval_calls:.2f} "
        f"query_avg={dbg['query_error_sum'] / eval_calls:.2f} "
    ...
    try:
        ...
        while gens_done < int(generations):
            ...
            for isl, st in enumerate(island_states):
                job_cfg = dict(cfg)
                job_cfg['island_idx'] = int(isl)
                job_cfg['evolution_progress'] = float(min(1.0, max(0.0, (gens_done + 0.5 * chunk_gens) / max(1, int(generations)))))
            ...
            try:
                main_eval_for_llm.evolution_progress = float(min(1.0, max(0.0, gens_done / max(1, int(generations)))))
            except Exception:
                pass
