...
class CMSketchEvaluator:
    ...
    def analyze_query_tree(self, query_tree):
        ...
        read_names = {"query_date", "cnt_rdstate"}
        forbidden = {
            "update_count", "write_count",
            "updatecount_if", "writecount_if",
            "update_state", "writestate_if",
    ...
    def _ast_effect_summary(self, root):
        ...
        return {
            ...
            "query_real_read_calls": int(names["query_date"]) + int(names["cnt_rdstate"]),
    ...
    def _ast_pattern_summary(self, which: str, root):
        ...
        def walk(nd, parent_name=None, child_pos=None):
            ...
            # update: 所有真实写入都必须是 base 的直接孩子，不能藏在任何参数/比较/算术里
            if which == "update" and name in write_names:
                if parent_name != "base":
                    out["bad_write_context"] += 1

            # update: write_count / update_count 的写入值不应该是布尔/比较型子树；
            # 这里只记结构坏味道，后面走软惩罚，不做 early cut
            ...
            # query: base_sel 的三个值分支里，至少两个分支要有真实合法读；
            #        这里的真实合法读 = query_date 或 cnt_rdstate；
            #        值分支里裸常量越多，越像退化 query
            if which == "query" and name == "base_sel" and len(nd["children"]) == 4:
                ...
                for ch in nd["children"][1:4]:
                    ...
                    if int(info["names"]["query_date"]) > 0 or int(info["names"]["cnt_rdstate"]) > 0:
    ...
    def _merge_hard_case_state(self, mined_state):
        ...
        if json.dumps(old_state, sort_keys=True, ensure_ascii=False) != json.dumps(self.export_hard_case_state(),
    ...
    def _query_error_on_items(self, query_func, expected_freq, items=None, max_items=None,
    ...
    def _query_collapse_stats(self, query_func, present_items, absent_items, cap: int = 24):
        cap = max(1, int(cap))

        def _extract_items(items):
            out = []
            for rec in list(items)[:cap]:
        ...
        present_keys = _extract_items(present_items)
        absent_keys = _extract_items(absent_items)
        ...
        for item in present_keys:
            try:
                present_vals.append(float(query_func(item)))
            except Exception:
                continue

        for item in absent_keys:
            try:
                absent_vals.append(float(query_func(item)))
            except Exception:
                continue

        if present_vals:
            uniq_present = len(set(present_vals))
            mode_ratio = max(Counter(present_vals).values()) / max(1, len(present_vals))
            present_span = max(present_vals) - min(present_vals)
            present_mean = float(statistics.mean(present_vals))
        else:
            uniq_present = 0
            mode_ratio = 1.0
            present_span = 0.0
        ...
        absent_mean = float(statistics.mean(absent_vals)) if absent_vals else 0.0

        return {
            "uniq_present": int(uniq_present),
            "mode_ratio": float(mode_ratio),
            "present_span": float(present_span),
            "present_mean": float(present_mean),
            "absent_mean": float(absent_mean),
            "present_absent_gap": float(present_mean - absent_mean),
        }

    def _evaluate_individual_core(self, init_dex_tree, update_tree, query_tree, stage_idx: int = 2,
                                  return_case_vec: bool = False, return_hard_cases: bool = False):
        try:
            ...
            # 化简后若没有任何真实合法读，说明 query 已经退化
            if query_eff["query_real_read_calls"] == 0:
            ...
            elif query_eff["query_real_read_calls"] < 2:
                penalty += (2 - query_eff["query_real_read_calls"]) * 22000

            # ---- AST 级结构约束：按当前原语集语义约束 hash/path id / query 结构 ----
            ...
            if update_pat["bad_write_context"] > 0:
                ...
                if bwc >= 2:
                    ...
                    self.debug_stats["total_error_sum"] += cut_penalty
                    return _ret(cut_penalty)
            # bad_write_value_shape：只做软惩罚，不做 early cut
            if int(update_pat.get("bad_write_value_shape", 0)) > 0:
                bwv = int(update_pat["bad_write_value_shape"])
                penalty += bwv * 60000


            if query_pat["query_value_missing_reads"] > 0:
                penalty += int(query_pat["query_value_missing_reads"]) * 90000

            if query_pat["query_constant_branch"] > 0:
                penalty += int(query_pat["query_constant_branch"]) * 45000
            ...
            def create_pset_update(count_matrices, overflow_matrices, exec_stats):
                ...
                pset_update.addEphemeralConstant("rand_delta", functools.partial(random.choice, [-1, 1, 2, -2, 3, -3]),
                                                 int)
            ...
            collapse_stats = self._query_collapse_stats(
                query_func,
                present_items=stage_items[:min(len(stage_items), 24)],
                absent_items=absent_eval_items[:min(len(absent_eval_items), 24)],
                cap=24,
            )

            if collapse_stats["uniq_present"] <= 2:
                penalty += 180000
            elif collapse_stats["uniq_present"] <= 4:
                penalty += 90000

            if collapse_stats["mode_ratio"] >= 0.80:
                penalty += int((collapse_stats["mode_ratio"] - 0.80) * 400000)
            elif collapse_stats["mode_ratio"] >= 0.65:
                penalty += int((collapse_stats["mode_ratio"] - 0.65) * 200000)

            if collapse_stats["present_span"] <= 2.0:
                penalty += 90000
            elif collapse_stats["present_span"] <= 4.0:
                penalty += 40000

            if collapse_stats["present_absent_gap"] <= 1.0:
                penalty += 120000
            elif collapse_stats["present_absent_gap"] <= 3.0:
                penalty += 60000
    ...
    def evaluate_individual(self, init_dex_tree, update_tree, query_tree, return_case_vec: bool = False,
                            return_hard_cases: bool = False):
...
def _filter_llm_team_bank_with_evaluator(gp_ctx, evaluator, cfg):
    ...
    if kept:
        print(
            f"[LLM_TEAM_FILTER] total={len(team_bank)} kept={len(kept)} best_fit={kept[0]['fitness']:.6f} best_err={kept[0]['error']:.2f}",
            flush=True)
    else:
...
class PrimitiveSpecReference:
    ...
    QUERY_REAL_READ_NAMES = {"query_date", "cnt_rdstate"}
...
class LLMProposalEngine:
    ...
    def _build_report(self, evaluator, team=None, fit=None, err=None, case_vec=None, hard_cases=None,
                      extra_prompt_hints=None):
        if team is None:
    ...
    def prepare_phase_candidates(self, phase, gp_ctx, evaluator, base_team, existing_canon, limit,
                                 extra_prompt_hints=None):
        ...
        def _consume_raw(raw_list):
            ...
            for obj in parsed_teams:
                ...
                n_evaluated += 1
                if chk.get("warnings"):
                    self.logger.warn("runtime/reference conflict accepted by runtime compatibility",
    ...
    for rank, idx in enumerate(idxs):
        ...
        try:
            ...
            replace_individual_in_state(
                ...
                team,
                (fit, err, case_vec),
                source_meta={"phase": "seed", "source": spec.get("source", "offline_json"),
    ...
    for spec in candidate_specs:
        ...
        try:
            ...
            replace_individual_in_state(
                ...
                source_meta={"phase": "stagnation", "source": spec.get("source", "offline_json"),
                             "rationale": spec.get("rationale", "")},
                fec_key=chk["key"],
            )
...
def _init_island_state(cfg, island_idx: int):
    ...
    state = {
        ...
        "hard_case_state": evaluator.export_hard_case_state() if cfg.get("hard_case_replay", False) else {"version": 0,
                                                                                                          **evaluator._empty_hard_case_state()},
...
def _evolve_island_chunk(job):
    ...
    print(
        ...
        f"bad_write_val_total={dbg['bad_write_value_total']} "
        f"penalty_avg={dbg['penalty_sum'] / eval_calls:.2f} "
        f"query_avg={dbg['query_error_sum'] / eval_calls:.2f} "
...
def evolve_cmsketch(
    ...
    try:
        if bool(cfg.get("llm_enable", False)) and str(cfg.get("llm_mode", "none")) in {"seeds", "both"}:
            ...
            cfg["llm_seed_specs"] = [
                _serialize_team_spec(c["team"], rationale=c.get("rationale", ""),
    ...
    try:
        ...
        while gens_done < int(generations):
            ...
            improved = False
            for r in results:
                if (r['best_fitness'] > best_fitness) or (
                        r['best_fitness'] == best_fitness and r['best_error'] < best_error):
...
def _run_one_restart_job(job):
    ...
    try:
        with open(log_path, "w", encoding="utf-8") as lf, \
            ...
            print(
                f"[RUN {run_idx + 1}] seed={run_seed} stage1_dataset_mode={args_dict.get('stage1_dataset_mode', 'proxy')} proxy_mode_arg={run_proxy_mode}")
            ...
            print(
                f"[STAGE1] proxy_mode_arg={run_proxy_mode} best_fitness={best_fitness:.6f} best_error={best_error:.2f}")
