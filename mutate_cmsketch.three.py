...
class CMSketchEvaluator:
    ...
    def _make_proxy_stream_from_pool(self, pool, rng):
        target = max(0, self.pkts)
        if target == 0:
            return []
    ...
    def _load_proxy_stream(self):
        ...
        for t in range(retry_times):
            ...
            cand = self._make_proxy_stream_from_pool(pool, rng)
    ...
    def _count_calls(self, tree, names):
        s = str(tree)
        return sum(s.count(name + "(") for name in names)

    def check_hash_count(self, init_dex_tree) -> int:
        return self._count_calls(
            init_dex_tree,
            ["select_hash", "hash_salt", "hash_on_slice"]
        )

    def check_update_count(self, update_tree) -> int:
        return self._count_calls(
            update_tree,
            ["update_count", "write_count", "updatecount_if", "writecount_if"]
        )

    def check_query_count(self, query_tree) -> int:
        return self._count_calls(
            query_tree,
            ["query_date"]
        )
    ...
    def _make_fec_fingerprint(
        ...
        touched = []
        for item in self.test_data[:self.fec_probe_update_n]:
        ...
        present_sig = []
        present_items = list(self.expected_freq.items())[:self.fec_probe_present_n]
    ...
    def _build_kpart_views(self):
        ...
        self.kpart_pkt_cuts = [cut1, cut2, cut3]
        self.kpart_expected = [
            self._calc_freq_from_stream(self.test_data[:cut1]),
            self._calc_freq_from_stream(self.test_data[:cut2]),
            self._calc_freq_from_stream(self.test_data[:cut3]),
        ]
    ...
    def _evaluate_individual_core(self, init_dex_tree, update_tree, query_tree, stage_idx: int = 2):
        try:
            ...
            if stage_idx >= 2:
                cache_key = (str(init_dex_tree), str(update_tree), str(query_tree))
                cached = self.eval_cache.get(cache_key)
                if cached is not None:
                    return cached
            else:
                cache_key = (stage_idx, str(init_dex_tree), str(update_tree), str(query_tree))
            ...
            penalty = 0
            hash_calls = self.check_hash_count(init_dex_tree)
            update_calls = self.check_update_count(update_tree)
            query_calls = self.check_query_count(query_tree)
            ...
            sample_n = min(128, len(self.test_data))
            uniq_sum = 0.0
            for e0 in self.test_data[:sample_n]:
            ...
            stage_pkt_cuts = self.kpart_pkt_cuts
            ...
            for s in range(stage_idx + 1):
                cut = int(stage_pkt_cuts[s])
                try:
                    for item in self.test_data[prev_cut:cut]:
                        update_func(item)
                except Exception:
                    return _ret(2_000_000_000.0, fec_fp=fec_fp)

                avg_upd_ops = exec_stats["upd_ops"] / max(1, cut)
    ...
    def generate_complete_code(self, init_dex_tree, update_tree, query_tree):
        init_dex_str = str(init_dex_tree)
        update_str = str(update_tree)
        query_str = str(query_tree)
...
# 3 * 102 * 102
from init_dex_language import *  # 提供 list_3 / seg_row_index / cap_* / select_hash 等（与演化时一致）

rows_per_matrix = 102
cols_per_matrix = 102
planes = 3

# count_matrices[z][row][col]
count_matrices = [
    [[0 for _ in range(cols_per_matrix)] for _ in range(rows_per_matrix)]
    for _ in range(planes)
]
...
def basemin(a: int, b: int, c: int) -> float:
...
# ---- math helpers ----
def safe_add(a: int, b: int) -> int: return int(a) + int(b)
def safe_sub(a: int, b: int) -> int: return int(a) - int(b)
def safe_mul(a: int, b: int) -> int: return int(a) * int(b)
def safe_div(a: int, b: int) -> int: return int(a) // (int(b) if int(b) != 0 else 1)
def safe_min(a: int, b: int) -> int: return int(a) if int(a) < int(b) else int(b)
def safe_max(a: int, b: int) -> int: return int(a) if int(a) > int(b) else int(b)

def get_rows():
    return rows_per_matrix

def get_cols():
    return cols_per_matrix
...
def evolve_cmsketch(
        ...
        elite_rate: float = 0.05,  # 每代强制保留 top-elite_rate 的 team
        reset_prob: float = 0.10,  # 组件重置：随机把某一棵树整棵重采样（大步跳）
        reset_whole_prob: float = 0.02,  # 更强重置：直接重采样某组件，不做其它变异
        recombine_prob: float = 0.20,  # 组件重组：从另一 parent 借一棵树（打破共适应）
        mutation_prob: float = 0.90,  # 常规变异概率（对一个随机组件做一次变异）
        stagnation_limit: int = 10,  # 连续多少代无提升触发“爆发期”
        burst_gens: int = 4,  # 爆发持续代数
        immigrant_rate: float = 0.35,  # 爆发期每代向种群注入随机新个体比例（每岛）
        mixed_seeded_prob: float = 0.25,
    ...
    try:
        ...
        print(f"[Baseline CMS] E_base={E_base:.2f}  (标准CMS在当前采样流上的总绝对误差)")
    ...
    elite_size = max(1, int(round(population_size * float(elite_rate))))
    ...
    for gen in range(int(generations)):
        ...
        batch_steps = max(1, min(population_size, int(eval_workers) * 4))
        produced = 0
        ...
        while produced < population_size:
            ...
            for _ in range(cur_steps):
                for isl, st in enumerate(island_states):
                    ...
                    if burst_left > 0:
                        _reset_prob = min(0.5, _reset_prob + 0.25)
                        _recombine_prob = min(0.8, _recombine_prob + 0.30)
                        _mutation_prob = min(0.98, _mutation_prob + 0.08)
                        _reset_whole_prob = min(0.2, _reset_whole_prob + 0.05)
                    ...
                    if random.random() < _reset_whole_prob:
                        ...
                        if random.random() < _recombine_prob:
        ...
        if best_error < prev_best_error:
            ...
            if best_team is not None:
                print("当前最佳个体表达式：")
                print("init_dex: ", str(best_team['init_dex']))
                print("update: ", str(best_team['update']))
                print("query: ", str(best_team['query']))
            no_improve = 0
        else:
            no_improve += 1
...
def _run_one_restart_job(job):
    ...
    try:
        with open(log_path, "w", encoding="utf-8") as lf, \
            ...
            print(f"[RUN {run_idx + 1}] seed={run_seed} proxy_mode={run_proxy_mode}")
            ...
            print(f"[STAGE1] proxy_mode={run_proxy_mode} best_fitness={best_fitness:.6f} best_error={best_error:.2f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--elite_rate", type=float, default=0.05, help="每代强制保留的精英比例")
    ...
    parser.add_argument("--recombine_prob", type=float, default=0.20, help="组件重组概率（从随机 donor 借一棵树）")
    ...
    parser.add_argument("--burst_gens", type=int, default=4, help="爆发期持续代数")
    parser.add_argument("--immigrant_rate", type=float, default=0.35, help="爆发期每代注入 immigrants 的比例（每岛）")
    parser.add_argument(
        "--mixed_seeded_prob",
        type=float,
        default=0.25,
